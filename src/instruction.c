...
int mkdir(DirectoryTree *dirTree, char *cmd)
{
    ...
    if (cmd[0] == '-')
    {
        if (strcmp(cmd, "-p") == 0)
        {
            ...
            while (str != NULL)
            {
                ...
                if (val != 0)
                {
                    make_directory(dirTree, str, 'd');
                    move_current_tree(dirTree, str);
                }
                str = strtok(NULL, "/");
            }
        ...
        else if (strcmp(cmd, "-m") == 0)
        {
            ...
            if (str[0] - '0' < 8 && str[1] - '0' < 8 && str[2] - '0' < 8 && strlen(str) == 3)
            {
                ...
                val = make_directory(dirTree, str, 'd');
                if (val == 0)
                {
                    tmpNode = is_exist_directory(dirTree, str, 'd');
    ...
    else
    {
        ...
        if (str == NULL)
        {
            strncpy(tmp, cmd, MAX_DIRECTORY_SIZE);
            if (strstr(cmd, "/") == NULL)
            {
                make_directory(dirTree, cmd, 'd');
                return 0;
            }
            else
            {
                strncpy(tmp2, get_directory(cmd), MAX_DIRECTORY_SIZE);
                val = move_directory_path(dirTree, tmp2);
                if (val != 0)
                {
        ...
        else
        {
            ...
            else
            {
                ...
                tmpNode = is_exist_directory(dirTree, NewNode->name, 'd');
...
int rm(DirectoryTree *dirTree, char *cmd)
{
    ...
    if (cmd[0] == '-')
    {
        if (strcmp(cmd, "-r") == 0)
        {
            ...
            if (strstr(str, "/") == NULL)
            {
                ...
                {
                    if (is_node_has_permission(dirTree->current, 'w') != 0 || is_node_has_permission(tmpNode, 'w') != 0)
                    {
                        printf("rm: '%s'디렉터리 또는 파일을 지울 수 없습니다: 허가거부\n", str);
                        return -1;
                    }
                    remove_directory(dirTree, str);
                }
            }
            else
            {
                ...
                tmpNode = is_exist_directory(dirTree, tmp3, 'd');
                if (tmpNode == NULL)
                {
                    printf("rm: '%s'를 지울 수 없음: 그런 파일이나 디렉터리가 없습니다\n", tmp3);
        ...
        else if (strcmp(cmd, "-f") == 0)
        {
            ...
            if (strstr(str, "/") == NULL)
            {
                tmpNode = is_exist_directory(dirTree, str, 'f');
                tmpNode2 = is_exist_directory(dirTree, str, 'd');
            ...
            else
            {
                ...
                tmpNode = is_exist_directory(dirTree, tmp3, 'f');
                tmpNode2 = is_exist_directory(dirTree, tmp3, 'd');
    ...
    else
    {
        ...
        if (strstr(cmd, "/") == NULL)
        {
            tmpNode = is_exist_directory(dirTree, cmd, 'f');
            tmpNode2 = is_exist_directory(dirTree, cmd, 'd');
...
int cd(DirectoryTree *dirTree, char *cmd)
{
    ...
    {
        tmpNode = is_exist_directory(dirTree, cmd, 'd');
        if (tmpNode != NULL)
        {
        ...
        val = move_directory_path(dirTree, cmd);
        if (val != 0)
...
int pwd(DirectoryTree *dirTree, Stack *dirStack, char *cmd)
{
    ...
    if (cmd == NULL)
    {
        print_directory_path(dirTree, dirStack);
    }
    else if (cmd[0] == '-')
    {
        if (strcmp(cmd, "--help") == 0)
...
int ls(DirectoryTree *dirTree, char *cmd)
{
    ...
    if (cmd[0] == '-')
    {
        if (strcmp(cmd, "-al") == 0 || strcmp(cmd, "-la") == 0)
        {
            ...
            {
                tmpNode = dirTree->current;
                val = move_directory_path(dirTree, str);
                if (val != 0)
                    return -1;
            }
            list_directory(dirTree, 1, 1);
        }
    ...
    **/
    ...
    else if (cmd[0] == '-')
    {
        if (strcmp(cmd, "-n") == 0)
        {
            ...
            if (strstr(str, "/") == NULL)
            {
                ...
                tmpNode = is_exist_directory(dirTree, str, 'd');
                tmpNode2 = is_exist_directory(dirTree, str, 'f');
            ...
            else
            {
                ...
                tmpNode = is_exist_directory(dirTree, tmp3, 'd');
                tmpNode2 = is_exist_directory(dirTree, tmp3, 'f');
    ...
    else
    {
        ...
        if (strstr(cmd, "/") == NULL)
        {
            ...
            tmpNode = is_exist_directory(dirTree, cmd, 'd');
            tmpNode2 = is_exist_directory(dirTree, cmd, 'f');
            if (tmpNode == NULL && tmpNode2 == NULL)
            {
                printf("cat: '%s': 그런 파일이나 디렉터리가 없습니다\n", cmd);
                return -1;
...
int find_(DirectoryTree *dirTree, char *cmd)
{
    ...
    if (cmd == NULL)
    {
        find_directory(dirTree, dirTree->current->name, 1);
        return 0;
    }
    else if (cmd[0] == '-')
    {
...
void instruction(DirectoryTree *dirTree, char *cmd)
{
    ...
    if (strcmp(str, "mkdir") == 0)
    {
        ...
        if (val == 0)
        {
            save_directory(dirTree, gp_directoryStack);
        }
    }
    ...
    {
        str = strtok(NULL, " ");
        pwd(dirTree, gp_directoryStack, str);
    }
